namespace Shop.Application.Primitives.Result;
...
public static class ResultExtensions
{
    ...
    public static Result<TOut> Map<TIn, TOut>(this Result<TIn> result, Func<TIn, TOut> func) =>
        result.IsSuccess ? func(result.Value) : Result.Failure<TOut>(result.Error, result.Errors);
    ...
    public static async Task<Result> Bind<TIn>(this Result<TIn> result, Func<TIn, Task<Result>> func) =>
        result.IsSuccess ? await func(result.Value) : Result.Failure(result.Error, result.Errors);
    ...
    public static async Task<Result<TOut>> Bind<TIn, TOut>(this Result<TIn> result, Func<TIn, Task<Result<TOut>>> func) =>
        result.IsSuccess ? await func(result.Value) : Result.Failure<TOut>(result.Error, result.Errors);
    ...
    /// </returns>
    public static async Task<T> Match<T>(this Task<Result> resultTask, Func<T> onSuccess, Func<Error, Error[]?, T> onFailure)
    {
        Result result = await resultTask;

        return result.IsSuccess ? onSuccess() : onFailure(result.Error, result.Errors);
    }
        ...
        Func<Error, Error[]?, TOut> onFailure)
    {
        ...
        return result.IsSuccess ? onSuccess(result.Value) : onFailure(result.Error, result.Errors);
    }
}
